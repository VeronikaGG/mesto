export class Card {
  ...
  generateCard() {
    ...
    //лайк
    // (this._likes.find((user) => user._id === this._userId)) this._handleLikeButton();

    // if (this._likes.find((user) => user._id === this._userId)) this._handleLikeClick();
  ...
  toggleLikes(data) {
    ...
    this._cardLikesCounter.textContent = data._likes.length;
  }
